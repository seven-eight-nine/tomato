...
public delegate void FlowScopeEnterHandler<in T>(T state) where T : class, IFlowState;

/// <summary>
/// スコープから出た時のハンドラ（ステートレス）。
/// </summary>
...
/// <param name="state">状態オブジェクト</param>
/// <param name="result">子ノードの実行結果</param>
public delegate void FlowScopeExitHandler<in T>(T state, NodeStatus result) where T : class, IFlowState;

/// <summary>
/// 入った瞬間/出た瞬間にコールバックを発火するデコレータノード（ステートレス）。
/// OnEnter: 初回Tick時のみ発火。
/// OnExit: Success/Failureになった時のみ発火（Running中は発火しない）。
/// 再帰呼び出しをサポート（呼び出し深度ごとに状態を管理）。
/// </summary>
public sealed class ScopeNode : IFlowNode
{
    private const int InitialCapacity = 4;

    private readonly IFlowNode _child;
    private readonly FlowScopeEnterHandler? _onEnter;
    private readonly FlowScopeExitHandler? _onExit;
    private readonly List<bool> _hasStartedStack;

    /// <summary>
    /// ScopeNodeを作成する。
    /// </summary>
    /// <param name="onEnter">入った瞬間に発火するコールバック（nullも可）</param>
    /// <param name="onExit">出た瞬間に発火するコールバック（nullも可）</param>
    /// <param name="child">子ノード</param>
    public ScopeNode(FlowScopeEnterHandler? onEnter, FlowScopeExitHandler? onExit, IFlowNode child)
    {
...
/// <summary>
/// 入った瞬間/出た瞬間にコールバックを発火するデコレータノード（型付き）。
/// </summary>
/// <typeparam name="T">状態の型</typeparam>
public sealed class ScopeNode<T> : IFlowNode where T : class, IFlowState
{
    private const int InitialCapacity = 4;

    private readonly IFlowNode _child;
    private readonly FlowScopeEnterHandler<T>? _onEnter;
    private readonly FlowScopeExitHandler<T>? _onExit;
    private readonly List<bool> _hasStartedStack;
    ...
    /// <param name="onExit">出た瞬間に発火するコールバック（nullも可）</param>
    /// <param name="child">子ノード</param>
    public ScopeNode(FlowScopeEnterHandler<T>? onEnter, FlowScopeExitHandler<T>? onExit, IFlowNode child)
    {
        _child = child ?? throw new ArgumentNullException(nameof(child));
        _onEnter = onEnter;
        _onExit = onExit;
